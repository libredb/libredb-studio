...
Prometheus is the newest, and the one that is not SQL at all: it speaks PromQL over the Prometheus HTTP API, browses metrics, rules and scrape targets, and is read-only because Studio calls none of the server's write or admin endpoints.
...
| **Prometheus** | none, HTTP (the Prometheus HTTP API, port 9090) | PromQL editor whose text reaches the server unchanged, results in the grid and the chart tab (a range draws one line per series), a metric browser with label names as columns and metadata as the source, rule groups with recording and alerting rules (a firing alert is marked in the tree), scrape pools and targets (a down target is marked), and health, version, uptime and TSDB statistics. Read-only by design: no admin API, no remote write, no EXPLAIN (the parse endpoint is experimental) and no maintenance. A credential over plain HTTP is sent rather than refused, so enable TLS across a network you do not control |
...
> VictoriaMetrics runs PromQL through the Prometheus driver and lists its metrics with their label columns, but its rule folders are empty, because a single-node server evaluates no rules; the overview, storage and table statistics, a metric's source, and the scrape pool and target folders fail, because it does not serve three of the Prometheus endpoints they read and answers three others without a field the driver requires; and a string expression returns no rows, while no PromQL info or warning appears beside any result.
