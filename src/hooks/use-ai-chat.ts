  ...
  const handleAiSubmit = useCallback(
    async (e?: React.FormEvent) => {
      ...
      try {
        ...
        if (schemaContext) {
          try {
            const topTables = [...parsedSchema].sort((a, b) => (b.rowCount || 0) - (a.rowCount || 0)).slice(0, 100);

            filteredSchemaContext = topTables
              .map((table) => {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "An unexpected error occurred while communicating with the AI.";
