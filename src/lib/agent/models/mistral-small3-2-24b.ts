...
export const MISTRAL_SMALL3_2_24B: AgentModelProfile = {
  ...
  /*
    ...
    identical shape refusals collapsing to one; `granite4.1:3b` earned it twice, its report
    landing where two refusals had stopped it and then the cell locking once
    `recommend_change` carried one too. This model shows the same shape of loss — a refused
    `present_answer` on data-analysis, and query-optimization spread across three shortfalls —
    but the run measured after switching it on never reached a shape refusal, so the example is
    still unproven here.
  */
