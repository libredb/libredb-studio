 ...
 *
 * `spec` is optional because the last group never resolves the kind before this call, and
 * an undeclared kind there must keep reading as the bare shape rather than as a new
 * failure. Every caller that has the spec in hand passes it, and `kind` - not `spec.id` -
 * stays in the message because that is what seven of the nine copies printed.
 */
export function assertObjectPathShape(
  capabilities: ProviderCapabilities,
  spec: ObjectKindSpec | undefined,
  kind: string,
