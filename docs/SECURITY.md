...
| 0.3 | Every route that reaches a database or an LLM verifies its caller in its own handler: a user session, or for the two machine surfaces a server-minted credential of their own, the agent drive callback's single-purpose token and the MCP endpoint's scoped bearer token | Implemented | [`src/lib/api/require-session.ts`](../src/lib/api/require-session.ts), [`src/lib/agent/drive-token.ts`](../src/lib/agent/drive-token.ts), [`src/lib/mcp/bearer.ts`](../src/lib/mcp/bearer.ts), [`src/lib/mcp/token.ts`](../src/lib/mcp/token.ts), [`src/app/api/mcp/route.ts`](../src/app/api/mcp/route.ts) | [`tests/security/route-auth.test.ts`](../tests/security/route-auth.test.ts), [`tests/api/agent/drive.test.ts`](../tests/api/agent/drive.test.ts), [`tests/security/mcp-auth.test.ts`](../tests/security/mcp-auth.test.ts) |
...
| 0.6 | The host and port of an HTTP-based database connection address only that server and the transport's own paths, and no such transport follows a redirect | Implemented | [`src/lib/db/http/endpoint.ts`](../src/lib/db/http/endpoint.ts), [`src/lib/db/providers/sql/clickhouse/http-transport.ts`](../src/lib/db/providers/sql/clickhouse/http-transport.ts), [`src/lib/db/providers/sql/druid/http-transport.ts`](../src/lib/db/providers/sql/druid/http-transport.ts), [`src/lib/db/providers/sql/search/http-transport.ts`](../src/lib/db/providers/sql/search/http-transport.ts), [`src/lib/db/providers/sql/trino/http-transport.ts`](../src/lib/db/providers/sql/trino/http-transport.ts), [`src/lib/db/providers/sql/libsql/hrana-transport.ts`](../src/lib/db/providers/sql/libsql/hrana-transport.ts), [`src/lib/db/providers/document/couchbase/http-transport.ts`](../src/lib/db/providers/document/couchbase/http-transport.ts), [`src/lib/db/providers/timeseries/prometheus/index.ts`](../src/lib/db/providers/timeseries/prometheus/index.ts), [`src/lib/db/providers/timeseries/prometheus/request.ts`](../src/lib/db/providers/timeseries/prometheus/request.ts) | [`tests/unit/db/http/endpoint.test.ts`](../tests/unit/db/http/endpoint.test.ts), [`tests/unit/db/clickhouse/http-transport.test.ts`](../tests/unit/db/clickhouse/http-transport.test.ts), [`tests/unit/db/druid/http-transport.test.ts`](../tests/unit/db/druid/http-transport.test.ts), [`tests/unit/db/search/http-transport.test.ts`](../tests/unit/db/search/http-transport.test.ts), [`tests/unit/db/trino/http-transport.test.ts`](../tests/unit/db/trino/http-transport.test.ts), [`tests/unit/db/libsql/hrana-transport.test.ts`](../tests/unit/db/libsql/hrana-transport.test.ts), [`tests/unit/db/couchbase/http-transport.test.ts`](../tests/unit/db/couchbase/http-transport.test.ts), [`tests/unit/db/prometheus/provider-endpoint.test.ts`](../tests/unit/db/prometheus/provider-endpoint.test.ts), [`tests/unit/db/prometheus/request.test.ts`](../tests/unit/db/prometheus/request.test.ts) |
...
| 1.2 | Login, AI and database-reaching routes are rate limited | Implemented | [`src/lib/api/rate-limit.ts`](../src/lib/api/rate-limit.ts), [`src/lib/mcp/preprocess.ts`](../src/lib/mcp/preprocess.ts) | [`tests/security/rate-limit-keying.test.ts`](../tests/security/rate-limit-keying.test.ts), [`tests/security/rate-limit-routes.test.ts`](../tests/security/rate-limit-routes.test.ts) |
| 1.3 | State-changing requests are checked against the deployment's own origin, and every request to `/api/mcp`, whatever its method, against the MCP Origin allowlist, with its `Host` checked on a loopback bind | Implemented | [`src/lib/api/origin-check.ts`](../src/lib/api/origin-check.ts), [`src/proxy.ts`](../src/proxy.ts), [`src/lib/mcp/origin-policy.ts`](../src/lib/mcp/origin-policy.ts), [`src/app/api/mcp/route.ts`](../src/app/api/mcp/route.ts) | [`tests/security/csrf-origin.test.ts`](../tests/security/csrf-origin.test.ts), [`tests/security/mcp-origin-host.test.ts`](../tests/security/mcp-origin-host.test.ts) |
| 1.4 | Authentication transitions and denials are audited | Partial | [`src/lib/audit.ts`](../src/lib/audit.ts), [`src/lib/api/require-session.ts`](../src/lib/api/require-session.ts), [`src/lib/mcp/bearer.ts`](../src/lib/mcp/bearer.ts), [`src/lib/mcp/origin-policy.ts`](../src/lib/mcp/origin-policy.ts), [`src/proxy.ts`](../src/proxy.ts) | [`tests/security/auth-audit.test.ts`](../tests/security/auth-audit.test.ts), [`tests/security/mcp-auth.test.ts`](../tests/security/mcp-auth.test.ts), [`tests/security/mcp-origin-host.test.ts`](../tests/security/mcp-origin-host.test.ts) |
...
| 3.7 | Every MCP tool call is audited under one correlation id, its decision before any provider is reached, and a call whose decision cannot be recorded does not run | Implemented | [`src/lib/mcp/audit.ts`](../src/lib/mcp/audit.ts), [`src/lib/mcp/output.ts`](../src/lib/mcp/output.ts), [`src/lib/mcp/preprocess.ts`](../src/lib/mcp/preprocess.ts), [`src/lib/mcp/tools/list-connections.ts`](../src/lib/mcp/tools/list-connections.ts), [`src/lib/mcp/tools/inspect-schema.ts`](../src/lib/mcp/tools/inspect-schema.ts), [`src/lib/mcp/tools/run-read-query.ts`](../src/lib/mcp/tools/run-read-query.ts), [`src/lib/audit.ts`](../src/lib/audit.ts) | [`tests/security/mcp-audit.test.ts`](../tests/security/mcp-audit.test.ts), [`tests/integration/mcp/run-read-query.test.ts`](../tests/integration/mcp/run-read-query.test.ts), [`tests/integration/mcp/inspect-schema.test.ts`](../tests/integration/mcp/inspect-schema.test.ts) |
...
**0.3.** Two routes verify something other than a user session, and both are named here rather than left to the test files.
`POST /api/agent/drive` is the callback that asks this server to pick an agent run up again, and its caller is the durable transport, not a person.
It verifies a credential this server minted: single-purpose, valid for a minute, naming one run, and signed with a key derived from `JWT_SECRET` rather than with `JWT_SECRET` itself, so it is not a session and cannot become one.
It grants nothing beyond continuing that run: what the run may read is decided by the actor recorded in the run's own ledger, never by the credential and never by the request body.
`/api/mcp` is the MCP endpoint, which an AI client of the user's own calls with a token minted for that user on the settings screen.
...
choice (see "Known limits"). What it guarantees is that the value in the `host` and `port` fields is
the whole of that choice: a host must be a hostname or an IP literal, a port an integer from 1 to
...
**3.7.** The MCP endpoint does not use the agent pipeline of row 3.5, so its audit trail is a control of its own.
Each tool call writes a decision event before any provider is reached and, when the decision allows the call, an outcome event after it, both under one correlation id.
A decision that cannot be written stops the call before acquisition, and an outcome that cannot be written withholds the result; either way the client reads one fixed sentence, and neither the sink's message nor a driver's.
Arguments that fail a tool's input schema are refused by the SDK before any handler runs, so the route records that refusal after the SDK has answered.
Only closed vocabularies are recorded, with one stated departure: the user is the MCP token's verified username rather than a mode and role label, because who ran a query is what a per-user machine credential exists to answer.
A connection is recorded by its bare seed id and only once it resolved; the connection id a client sent is never recorded.
