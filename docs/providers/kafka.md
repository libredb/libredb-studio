...
Each exchange holds a thread of the runtime's pool for as long as its count takes, measured at about 2.9 million SHA-512 iterations a second under Node 24.14.0 and 3.3 million under Bun 1.4.2, so the largest count the runtime's PBKDF2 accepts, 2,147,483,647, takes about 11 to 12 minutes, and a larger one fails the authentication at once.
...
- `{ "timestamp": "<ISO-8601>" }`: each partition from the first offset whose timestamp is at or after the instant, through `listOffsetsWithTimestamps`.
  ...
  It is written as `YYYY-MM-DDThh:mm`, with optional seconds of at most three decimals, then `Z`, `+hh:mm` or `-hh:mm`; a finer fraction is refused rather than cut, because `Date.parse` cuts it to the millisecond, and a record less than a millisecond before the instant would then read as at or after it.
...
An error API 69 answers for the group is read by its protocol error name, as [§10](#10-error-handling) reads any other and never by the broker's text: a refused group or topic is an `AuthenticationError` naming it, a coordinator that moved or is still loading a `QueryError` naming the error, and `GROUP_ID_NOT_FOUND`, for an id with no consumer-protocol group, a `QueryError` saying so.
