...
| RisingWave | `postgres` | Query editor only | RisingWave 3.0.3 | **Re-probed after #38680: the monitoring dashboard now loads** with every statistic marked unavailable rather than erroring the whole page (it has no pg statistics catalog at all, and a parameterised `LIMIT` is rejected, so slow-query/session panels stay empty specifically). The object browser remains unavailable, and re-probing found a different cause than Materialize's: the schema query's `LEFT JOIN pg_class ON (...)::regclass` fails to bind (`missing FROM-clause entry for table c`), not the `MATERIALIZED` keyword - not yet worked around. |
