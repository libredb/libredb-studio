...
a server-side enum with eighteen members, so what it discloses is which of eighteen engines this
...
sixteen it invokes `db.schema.read`, which walks the connection's own OBJECT SURFACE — the same
...
**Sixteen counts type-ids the factory can build, not engines a user would name**: `SHIPPED` holds
eighteen, `CATALOG_PLANS` serves two of them, and the remainder is what this second reading covers.
Every other count said about grounding in these docs counts the same thing. libSQL is one of the
sixteen and not one of the two: it speaks SQLite's dialect, but the read-only catalog path
...
arrives as `mysql`, and it is that type-id that is counted. And two of the sixteen, the embedded
...
On the eleven engines that declare no foreign keys at all (MongoDB, Redis, LibreDB, Druid, ClickHouse, Couchbase, Trino, Cassandra, Elasticsearch, OpenSearch and Prometheus) this block carries no relations and says why: the engine has no such constraint to
...
| `maxStatementsPerRun` | 18-45, by workflow | Reads per run, folded across its drives (#999), grounding reads and repairs included — the composed catalog reads and, since #414, the one `db.schema.read` call that replaces them on the other sixteen. The figures did not move for it: that path is the cheapest of the three, so nothing had to be bought (`docs/AGENT.md`, the budget arithmetic) |
