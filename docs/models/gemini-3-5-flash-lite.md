...
Served through **Gemini API**.

The one hosted model of the ten, and the fastest: a median run of ten seconds against a median of thirty-two for the local models. Its numbers were taken exactly as theirs were — same six surfaces, five consecutive passes each, same turn limit — so the ten are one table rather than a local one with a cloud footnote.
