...
- **読み取り専用。しかもデータベース自身が保証する**：すべての文はアプリの他の部分と同じ操作パイプラインを
  通り、読み取り専用の実行プロファイルで動きます（PostgreSQLでは読み取り専用トランザクション、SQLiteでは
  文ごとに`PRAGMA query_only`を再宣言）。書き込みとDDLはデータベースに届く前に拒否され、
  `EXPLAIN ANALYZE`は文を実際に実行してしまうため既定で不許可です。
...
- **モデルは自分のもの**：Gemini（既定）、OpenAI、Ollama、またはOpenAI互換の任意のエンドポイント。Ollamaでは
  ツール呼び出しに対応したモデルが必要で、それはベンダーの資料ではなく実際のプローブで確かめます。
