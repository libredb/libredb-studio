...
function chatToolCallDelta(name: string, argumentsChunk: string): Record<string, unknown> {
  return {
    tool_calls: [{ index: 0, id: "call_1", type: "function", function: { name, arguments: argumentsChunk } }],
  };
}
 ...
 */
export function chatToolCallStream(name: string, argumentsJson: string): Response {
  return sseResponse(
    chatChunk({ role: "assistant", content: null, ...chatToolCallDelta(name, argumentsJson) }),
