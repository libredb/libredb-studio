...
describe('POST /api/ai/index-advisor', () => {
  beforeEach(() => {
  ...
  test('returns streaming response with data', async () => {
    const req = createMockRequest('/api/ai/index-advisor', {
      method: 'POST',
      body: {
        slowQueries: [{ query: 'SELECT * FROM users WHERE email = $1', avgTime: 150 }],
        indexStats: [{ name: 'idx_users_pkey', scans: 500 }],
        tableStats: [{ name: 'users', rows: 50000 }],
