...
describe("StudioMobileHeader", () => {
  ...
  test('displays "Select DB" when no active connection', () => {
    const html = renderToStaticMarkup(<StudioMobileHeader {...defaultProps} activeConnection={null} />);
  ...
  test("hides user trigger button when user is null", () => {
    const html = renderToStaticMarkup(<StudioMobileHeader {...defaultProps} user={null} />);
  ...
  test("disables RUN button when no active connection", () => {
    const html = renderToStaticMarkup(
      <StudioMobileHeader {...defaultProps} activeConnection={null} isExecuting={false} />,
    );
  ...
  test("always renders Row 1 regardless of activeMobileTab", () => {
    for (const tab of ["database", "schema", "editor"] as const) {
      const html = renderToStaticMarkup(<StudioMobileHeader {...defaultProps} activeMobileTab={tab} />);
  ...
  test("pulse states are mutually exclusive classes", () => {
    const healthyHtml = renderToStaticMarkup(<StudioMobileHeader {...defaultProps} connectionPulse="healthy" />);
    const degradedHtml = renderToStaticMarkup(<StudioMobileHeader {...defaultProps} connectionPulse="degraded" />);
    const errorHtml = renderToStaticMarkup(<StudioMobileHeader {...defaultProps} connectionPulse="error" />);
