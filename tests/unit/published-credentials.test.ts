...
describe("the documentation publishes no credential that works", () => {
  ...
  test("reads a password out of a login body, and leaves a schema alone", () => {
    ...
    expect(caught(`{"host": "127.0.0.1", "user": "example-user", "password": "example-fake-db-pw"}`)).toEqual([]);
    expect(caught(`{ host: 'h', user: 'example-user', password: 'example-fake-db-pw' }`)).toEqual([]);
    expect(
