...
import { type LLMConfig, type LLMStreamOptions, LLMConfigError } from "@/lib/llm/types";
...
describe("BaseLLMProvider", () => {
  ...
  describe("validate()", () => {
    test("calls validateConfig", () => {
      ...
      const provider = new TestProvider(makeConfig({ provider: "gemini", apiKey: "key", model: "model" }));
  ...
  describe("getModel()", () => {
    ...
    test("returns options.model when provided", () => {
      ...
      const model = provider.exposedGetModel(makeStreamOptions({ model: "options-model" }));
  ...
  describe("ensureApiKey()", () => {
    ...
    test("a named provider missing its key is an unfinished setup, not an absence", () => {
      const provider = new ExposedProvider(makeConfig({ apiKey: undefined, providerExplicit: true }));
      try {
        provider.exposedEnsureApiKey();
      } catch (error) {
        expect((error as LLMConfigError).reason).toBeUndefined();
      }
    });
  });
  ...
  describe("ensureApiUrl()", () => {
    ...
    test("returns apiUrl when present", () => {
      const provider = new ExposedProvider(makeConfig({ apiUrl: "https://api.example.com/v1" }));
