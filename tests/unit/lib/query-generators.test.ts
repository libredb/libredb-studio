...
describe("generateTableQuery", () => {
  test("SQL previews leave their page bound to execution options", () => {
  ...
  test("Oracle previews leave their FETCH bound to execution options", () => {
  ...
  test("SQL Server previews leave their TOP bound to execution options", () => {
...
describe("ClickHouse (8123) generation", () => {
  ...
  test("the generated preview can receive the next page's execution bound", () => {
    const out = generateTableQuery(["events"], clickhouseCaps);
    const page = applyQueryLimit(out, 50, 50, {}, "clickhouse");
    expect(page.wasLimited).toBe(true);
    expect(page.sql).toBe("SELECT * FROM events LIMIT 50 OFFSET 50;");
  });
