...
describe("detectSensitiveColumnsFromConfig", () => {
  test("detects matching columns based on enabled patterns", () => {
    const result = detectSensitiveColumnsFromConfig(["id", "name", "email", "phone"], mockMaskingConfigEnabled);
  ...
  test("returns empty map when all patterns are disabled", () => {
    const result = detectSensitiveColumnsFromConfig(["email", "ssn", "phone"], mockMaskingConfigAllDisabledPatterns);
    expect(result.size).toBe(0);
  });

  test("matches case-insensitively", () => {
    const result = detectSensitiveColumnsFromConfig(["EMAIL", "Phone"], mockMaskingConfigEnabled);
    expect(result.has("EMAIL")).toBe(true);
    expect(result.has("Phone")).toBe(true);
  });
...
describe("applyMaskingToRows", () => {
  ...
  test("masks sensitive columns", () => {
    const sensitiveColumns = detectSensitiveColumnsFromConfig(["id", "email", "name"], mockMaskingConfigEnabled);
  ...
  test("does not mutate original rows", () => {
    ...
    const sensitiveColumns = detectSensitiveColumnsFromConfig(["id", "email"], mockMaskingConfigEnabled);
