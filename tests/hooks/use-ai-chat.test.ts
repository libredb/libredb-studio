...
describe("useAiChat", () => {
  ...
  test('table with no columns outputs "(none)"', async () => {
    const tablesNoColumns: ParsedTable[] = [{ name: "empty_table", rowCount: 0, columns: [] }];
  ...
  test("table with >10 columns appends ellipsis", async () => {
    ...
    const tablesMany: ParsedTable[] = [{ name: "wide_table", rowCount: 50, columns: manyColumns }];
  ...
  test("second submit sends conversationHistory", async () => {
    ...
    // First submit
    act(() => {
      result.current.setAiPrompt("first query");
    });
    await act(async () => {
      await result.current.handleAiSubmit();
    });
    ...
    // Second submit
    act(() => {
      result.current.setAiPrompt("second query");
    });
    await act(async () => {
      await result.current.handleAiSubmit();
    });
  ...
  test("first submit has no conversationHistory", async () => {
    ...
    act(() => {
      result.current.setAiPrompt("first");
    });
    await act(async () => {
      await result.current.handleAiSubmit();
    });
  ...
  test("onChange callback invoked with final response", async () => {
    ...
    act(() => {
      result.current.setAiPrompt("answer");
    });
    await act(async () => {
      await result.current.handleAiSubmit();
    });
